...
# for g in gesture:
aaa = 'file7'
file_name = data_folder+ aaa +'.bin'
    # file_name = data_folder + 'adc_data'+str(g) + '_Raw_0.bin'
...
g = 0
if output_data_type == 'RDI':
    # save_filename = output_data_type + 'G' + str(g)
    save_filename = aaa
    print("Save File：", save_filename)
    np.save(output_folder + save_filename, rdi)

elif output_data_type == 'RAI':
    # save_filename = output_data_type + 'G' + str(g)
    save_filename = aaa
    print("Save File：", save_filename)
    np.save(output_folder + save_filename, rai)
