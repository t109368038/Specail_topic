...
aoa_frame_1 = cdata[3]
aoa_chirp_1 = aoa_frame_1[20]

for phi in range(0, 60):
    for theta in range(-60, 60):
        beam_matrix = antenna_d * (cos_matrix * np.cos(theta) + sin_matrix * np.sin(theta))
        # print(beam_matrix)
        # print('===============')
        beam_factor = np.exp(-1j * beam_matrix * np.sin(phi) * np.pi * f_0 / c_0)
