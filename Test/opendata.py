...
data_folder ='D:\\train_data\\Matt_Yen\\3t4r - 複製\\'
# output_folder = '../data/Studio_data/2t4r_v132/'
# output_folder = 'C:\\Users\\user\\Downloads\\Matt_Yen\\after_p_v1\\'
output_folder = 'D:\\train_data\\Matt_Yen\\1t4rnewsuffle\\'

if not os.path.isdir(output_folder):
    os.makedirs(output_folder)
# randx = .random_int_list(0,12,12)
# print(randx)
# read bin data & process
arr = list(np.arange(1,121))
random.shuffle(arr)
for s in sense:
    counts = 0
    for temp in range(12):
        ...
        for g  in gesture:
            for tt in range(10):
                ...
                file_name = data_folder + 'adc_data_3t4r_' + str(s) + '_' + str(g) + '_' + str(arr[(tt+temp*10)]) +  \
                             '_process_0.bin'
        ...
        if output_data_type == 'RDI':
            save_filename = output_data_type + '_S' + str(s) + 'P' + str(counts)
