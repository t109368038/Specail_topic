...
rdi = DSP.Range_Doppler(data, 0, [64, 64])
rai = DSP.Range_Angle(data, 0, [64, 64, 32])
...
win_param = [10, 10, 2, 2]
cfar_rai = CA_CFAR(win_param, threshold=1.5, rd_size=[64, 32])
out = cfar_rai(rai[15, :, :])

plt.figure(1)
plt.imshow(np.flip(np.fft.fftshift(np.abs(out * rai[15, :, :]), axes=1)))
plt.figure(2)
plt.imshow(np.flip(np.fft.fftshift(np.abs(rai[15, :, :]), axes=1)))
